...
/** 사용자 알림 설정 조회 */
export const GET_notification_preferences = async (): Promise<ApiResponse<NotificationResponse>> => {
  const response = await api.get('/api/v1/users/notification-preferences');
  return response.data;
};

/** 사용자 알림 설정 변경 */
export const PUT_notification_preferences = async (data: NotificationRequest) => {
