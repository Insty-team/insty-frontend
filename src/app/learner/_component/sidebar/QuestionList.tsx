...
function QuestionList({ onSelect }: { onSelect: (id: number) => void }) {
	const { id } = useParams();
	...
	const filteredQuestions = questions.filter((q) => q.courseId === Number(id));

	return (
		<div className="flex-1 overflow-y-auto p-6">
			<button className="w-full mb-4 px-4 py-3 bg-gradient-to-r from-primary-green-300 to-primary-green-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg transform transition-all duration-200 ease-in-out flex items-center justify-center gap-2 cursor-pointer">
...
const questions = [
	...
	},
	{
		...
		courseId: 1,
		questionId: 5,
		title: "Homebrew로 설치한 패키지가 업데이트가 안 돼요, 어떻게 해야 할까요?",
		content:
	...
	},
	{
		...
		courseId: 1,
		questionId: 8,
		title: "Flutter에서 핫리로드가 안 먹히고 전체 빌드만 돼요. 해결법 있나요?",
		content:
