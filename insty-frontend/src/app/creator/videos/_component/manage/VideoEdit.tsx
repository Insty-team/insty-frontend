...
function VideoEdit({
	...
	return (
		<div>
			<h2 className="font-bold text-3xl mb-4">콘텐츠 수정</h2>
			<div className="flex w-full gap-9">
				<div className="flex flex-col w-2/5 min-w-[220px] max-w-[350px]">
					...
					<div className="mb-2 w-full h-36 bg-gray-scale-100 rounded-2xl flex items-center justify-center relative">
						<button className="absolute top-1 right-2 text-black-500">✕</button>
					</div>
					<div className="flex flex-col gap-3 mb-2 justify-center align-middle text-center">
						<button className="flex-1 py-2 bg-primary-blue-600 text-white rounded cursor-pointer">
							썸네일 선택
						</button>
						<button className="flex-1 py-2 border !border-primary-blue-600 text-primary-blue-600 rounded cursor-pointer">
							실습 자료 파일 선택
						</button>
						<button className="text-secondary-red-300 text-lg cursor-pointer">
							<span>업로드 영상 삭제</span>
				...
				<div className="flex-1 flex flex-col gap-4">
					<div>
						...
						<div className="flex gap-2">
							...
							/>
							<button className="px-3 py-1 bg-gray-100 border border-gray-200 rounded-2xl text-md cursor-pointer">
								AI추천 문장입력
							</button>
						</div>
					</div>
						...
						<div className="flex flex-wrap gap-2 mt-2">
							{tags.map((tag, idx) => (
								...
								>
									...
									>
										<Image src="/cancel.svg" alt="취소" width={16} height={16}  className="ml-2"/>
									</button>
								</span>
							))}
						</div>
